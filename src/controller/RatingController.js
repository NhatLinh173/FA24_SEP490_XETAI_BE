...
class RatingController {
    ...
    async addRating(req, res) {
        ...
        try {
            ...
            // Kiểm tra xem người đánh giá đã đánh giá tài xế này chưa
            const existingRating = await Rating.findOne({ userId, reviewerId });
            if (existingRating) {
                return res.status(400).json({ message: 'Bạn đã đánh giá tài xế này. Vui lòng sử dụng updateRating để sửa đánh giá.' });
            }
            ...
            const totalDriverRating = driverRatings.reduce((acc, r) => acc + r.value, 0);
            const averageDriverRating = totalDriverRating / driverRatings.length;

            // Cập nhật averageRating cho cả User và Driver
            await User.findByIdAndUpdate(userId, { averageRating: averageUserRating });
            await Driver.findOneAndUpdate({ userId }, { averageRating: averageDriverRating });
    ...
    async updateRating(req, res) {
        ...
        try {
            ...
            if (!existingRating) {
                return res.status(404).json({ message: 'Không tìm thấy đánh giá. Hãy sử dụng addRating để tạo mới.' });
            }
            ...
            return res.status(200).json({ message: 'Cập nhật đánh giá thành công', updatedRating });
        } catch (err) {
            console.error('Lỗi khi cập nhật đánh giá:', err);
            return res.status(500).json({ message: 'Lỗi khi cập nhật đánh giá', error: err });
        }
    }
    ...
    async getMyRatings(req, res) {
        ...
        try {
            ...
            return res.status(500).json({ message: 'Lỗi khi lấy đánh giá của tài xế', error: err });
        }
    }
    ...
    async getRatingsGivenByUser(req, res) {
        ...
        try {
            ...
            return res.json({ message: 'Lấy đánh giá đã thực hiện thành công', ratings });
        } catch (err) {
